...
JFK_total_fit <- kmeans(JFK_total_dat, centers = cluster_number)
JFK_total_dat['cluster'] <- JFK_total_fit$cluster
LG_total_fit <- kmeans(LG_total_dat, centers = cluster_number)
LG_total_dat['cluster'] <- LG_total_fit$cluster
Newark_total_fit <- kmeans(Newark_total_dat, centers = cluster_number)
Newark_total_dat['cluster'] <- Newark_total_fit$cluster

# JFK
map<-get_map(location='Manhatten', zoom=10)
gmap <- ggmap(map)
gmap <- gmap+geom_point(data=as.data.frame(JFK_total_dat), aes(x=longitude, y = latitude, color = rainbow(cluster_number)[cluster], alpha = 0.1))
gmap <- gmap+geom_point(data=as.data.frame(JFK_total_fit$centers), aes(x=longitude,y=latitude))
gmap

# LG
map<-get_map(location='Manhatten', zoom=10)
gmap <- ggmap(map)
gmap <- gmap+geom_point(data=as.data.frame(LG_total_dat), aes(x=longitude, y = latitude, color = rainbow(cluster_number)[cluster], alpha = 0.1))
